...
function candidateDirs() {
  ...
  out2.push(path.join(here, "wasm"));
  out2.push(path.resolve(here, "..", "..", "node_modules", "@vscode", "tree-sitter-wasm", "wasm"));
  try {
    const require2 = createRequire(import.meta.url);
    out2.push(path.dirname(require2.resolve("@vscode/tree-sitter-wasm/wasm/tree-sitter-ruby.wasm")));
  } catch {
  }
  return out2;
}
function grammarDir() {
  ...
  for (const dir of candidateDirs()) {
    if (fs2.existsSync(path.join(dir, "tree-sitter-ruby.wasm"))) {
      cachedDir = dir;
      return dir;
    }
  }
...
function readGrammar(name2) {
  return fs2.readFileSync(path.join(grammarDir(), `tree-sitter-${name2}.wasm`));
}
...
main().then(
  ...
  (err2) => {
    console.error(`nobble: ${err2 instanceof Error ? err2.message : String(err2)}`);
    process.exit(2);
  }
);
